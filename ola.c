...
void ordenarCodPostal(){
	...
	{
		for (j = i+1; j < nFinalAlunos;j++)
		{
			if(aluno_x[i].morada.codigoPostal_ > aluno_x[j].morada.codigoPostal_)
			{
				temp = aluno_x[i].morada.codigoPostal_;
				aluno_x[i].morada.codigoPostal_= aluno_x[j].morada.codigoPostal_;
				aluno_x[j].morada.codigoPostal_= temp;
			}
		}
	}
